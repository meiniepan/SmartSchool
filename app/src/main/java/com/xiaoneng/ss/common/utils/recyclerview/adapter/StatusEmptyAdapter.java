...
public class StatusEmptyAdapter extends BaseQuickAdapter<StatusRvEntity, BaseViewHolder> {


    public StatusEmptyAdapter(int layoutResId, @Nullable List<StatusRvEntity> data) {
    ...
    protected void convert(BaseViewHolder helper, StatusRvEntity item) {
        if (!TextUtils.isEmpty(item.msg)) {
            TextView tv_empty = helper.getView(R.id.tv_empty);
            if(null!=tv_empty) tv_empty.setText(item.msg);
        }
        if (item.drawableRes != 0) {
            ImageView imageView = helper.getView(R.id.iv_empty);
